...
class RatingWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle('Общий рейтинг')  # Set the window title
        ...
        WHERE length(marks) - length(replace(marks, '2', '')) = 0 AND length(marks) - length(replace(marks, '_', '')) = 0
        ORDER BY (length(marks) - length(replace(marks, '5', ''))) * 2
        + (length(marks) - length(replace(marks, '4', ''))) * 1 
        - (length(marks) - length(replace(marks, '3', ''))) 
        + social_activities DESC
        ''')
